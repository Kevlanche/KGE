...
import com.kevlanche.engine.game.state.impl.Position.X;
import com.kevlanche.engine.game.state.impl.Position.Y;
...
public class Box2dEntity extends BaseEntity {
	...
	public static final String CLASS_NAME = "box2dEntity";
	
	public Box2dEntity(Entity parent, World world) {
		...
		final Position.X x = new Position.X() {
		...
		final Position.Y y = new Position.Y() {
			public void set(float value) throws TypeException {
				final Transform trans = mBody.getTransform();
		...
		final Size.Width width = new Size.Width() {
			public void set(float value) throws TypeException {
		...
		final Size.Height height = new Size.Height() {
			public void set(float value) throws TypeException {
		...
		addPermanentState(position);
		addPermanentState(size);
		addPermanentState(rotation);
		addPermanentState(physics);
//		addPermanentState(new Rendering(new DrawableSrc(GdxAssetProvider.DEFAULT_IMAGE)));
	}
